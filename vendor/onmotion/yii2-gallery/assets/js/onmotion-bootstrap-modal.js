...
$(document).ready(function () {
	...
	$('a[role="modals-click"]').on('click', function (e) {
        e.preventDefault();
        $modal1.find('.modal-title').text($(this).attr('data-modal-title'));
        $modal1.find('.modal-body').text($(this).attr('data-modal-body'));
        $modal1.find('#modal-confirm-btn').attr('href', $(this).attr('href'));
        var method = $(this).attr('method');
        if (method == 'get') {
            $.ajax({
                type: 'get',
                ...
                dataType: 'json',
                beforeSend: function () {
                    $modal1.find('.modal-body').html(progressBar);
                },
                success: function (data) {
                    $modal1.find('.modal-title').text(data.content.title);
                    $modal1.find('.modal-body').html(data.content);
                },
                error: function (xhr, textStatus, errorThrown) {
                    console.log(xhr);
                    console.log(textStatus);
                    console.log(errorThrown);
                    $modal1.find('.modal-body').text(errorThrown);
                }
            });
        }
        $modal1.modal();
