 ...
 * @since 2.0
 */
class SluggableBehavior1 extends AttributeBehavior
{
    /**
     * @var string the attribute that will receive the slug value
     */
    public $slugAttribute = 'slug1';
    /**
    ...
    protected function getValue($event)
    {
        ...
        if ($this->attribute !== null) {
            if ($this->isNewSlugNeeded()) {
				...
				//print_r($slugParts);
				$slugParts[0] = $slugParts[0][2]['title'];
                $slug1 = $this->generateSlug($slugParts);
				//echo $slug1;
            } else {
                return $this->owner->{$this->slugAttribute};
            }
        } else {
            $slug1 = parent::getValue($event);
        }

        return $this->ensureUnique ? $this->makeUnique($slug1) : $slug1;
    }
     ...
     * @since 2.0.7
     */
    protected function makeUnique($slug1)
    {
        $uniqueSlug = $slug1;
        $iteration = 0;
        while (!$this->validateSlug($uniqueSlug)) {
            $iteration++;
            $uniqueSlug = $this->generateUniqueSlug($slug1, $iteration);
        }
        return $uniqueSlug;
    }
     ...
     */
    protected function validateSlug($slug1)
    {
        ...
        $model->clearErrors();
        $model->{$this->slugAttribute} = $slug1;
