 ...
 * @since 2.0
 */
class SluggableBehavior2 extends AttributeBehavior
{
    /**
     * @var string the attribute that will receive the slug value
     */
    public $slugAttribute = 'slug2';
    /**
    ...
    protected function getValue($event)
    {
        ...
        if ($this->attribute !== null) {
            if ($this->isNewSlugNeeded()) {
				...
				$slugParts[0] = $slugParts[0][3]['title'];
                $slug2 = $this->generateSlug($slugParts);
				//echo $slug2;
            } else {
                return $this->owner->{$this->slugAttribute};
            }
        } else {
            $slug2 = parent::getValue($event);
        }

        return $this->ensureUnique ? $this->makeUnique($slug2) : $slug2;
    }
     ...
     * @since 2.0.7
     */
    protected function makeUnique($slug2)
    {
        $uniqueSlug = $slug2;
        $iteration = 0;
        while (!$this->validateSlug($uniqueSlug)) {
            $iteration++;
            $uniqueSlug = $this->generateUniqueSlug($slug2, $iteration);
        }
        return $uniqueSlug;
    }
     ...
     */
    protected function validateSlug($slug2)
    {
        ...
        $model->clearErrors();
        $model->{$this->slugAttribute} = $slug2;
